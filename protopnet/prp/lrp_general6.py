...
class conv2d_beta0_wrapper_fct(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        #exit()
        # print(lrpignorebiastensor.item())
        return R,None, None
...
class conv2d_zbeta_wrapper_fct(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        # print("conv zebta:", print(torch.sum(R)) )
        #exit()
...
class sigmoid_wrapper_fct(torch.autograd.Function): # to be used with generic_activation_pool_wrapper_class(module,this)
    """
    We can implement our own custom autograd Functions by subclassing
    torch.autograd.Function and implementing the forward and backward passes
    which operate on Tensors.
...
class l2_lrp(torch.autograd.Function):
    ...
    #     prototypes = prototypes.repeat(1,1,i,j)
    #
    #     conv = conv.permute(0,2,3,1)
    #     prototypes = prototypes.permute(0,2,3,1)
    #
    #     l2 = (conv-prototypes)**2
    #     d = 1/(l2**2 + 1e-12)
    #
    #     denom = torch.sum(d,dim=-1) + 1e-12
    #     denom = torch.unsqueeze(denom,dim=-1).repeat(1,1,1,c) + 1e-12
    #     R = torch.div(d,denom)
    #
    #     grad_output = grad_output.repeat(c,1,1,1)
    #     grad_output = grad_output.permute(1, 2, 3, 0)
    #
    #     R = R * grad_output
    #
    #     ## Check at location p=1,i=1,j=1,c=1
    #     # print("Z", conv[1,1,1,1])
    #     # print("P", prototypes[1,1,1,1])
    #     #
    #     # print("R", R[1,1,1,1])
    #     # exit()
    #
