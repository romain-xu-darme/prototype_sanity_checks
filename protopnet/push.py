...
def get_prototype_infos(
    ...
    search_batch_size = dataloader.batch_size
    num_classes = ppnet_parallel.module.num_classes

    # Init prototype infos
    proto_infos = [{'id': p_idx, 'path': None, 'location': None, 'min_dist': np.inf} for p_idx in range(n_prototypes)]

    for push_iter, (search_batch_input, search_y) in enumerate(tqdm(dataloader)):
        start_index_of_search_batch = push_iter * search_batch_size

        # Inference
        with torch.no_grad():
            search_batch_input = search_batch_input.cuda()
            protoL_input_torch, proto_dist_torch = ppnet_parallel.module.push_forward(search_batch_input)
        proto_dist_torch = proto_dist_torch.detach().cpu().numpy()  # Shape N x P x H x W

        # Keep track on the label of each input in the batch
        class_to_img_index_dict = {key: [] for key in range(num_classes)}
        for img_index, img_y in enumerate(search_y):
            img_label = img_y.item()
            class_to_img_index_dict[img_label].append(img_index)
        ...
        for p_idx in range(n_prototypes):
            target_class = torch.argmax(ppnet_parallel.module.prototype_class_identity[p_idx]).item()
            if len(class_to_img_index_dict[target_class]) == 0:
                # No input in the batch is from this class, skip
                continue
            # Select distances of relevant inputs only
            proto_dist_sel = proto_dist_torch[class_to_img_index_dict[target_class]][:, p_idx, :, :]  # Shape n x H x W
            min_dist = np.amin(proto_dist_sel)
            if min_dist < proto_infos[p_idx]['min_dist']:
                # One input inside the batch is closer
                location = \
                    list(np.unravel_index(np.argmin(proto_dist_sel, axis=None),
                                          proto_dist_sel.shape))
                # Restore first index of location
                location[0] = class_to_img_index_dict[target_class][location[0]]
                n, h, w = location
                # Now proto_dist_torch[n, p_idx, h, w] = min_dist
                proto_infos[p_idx]['min_dist'] = min_dist
                proto_infos[p_idx]['location'] = (h, w)
                proto_infos[p_idx]['path'] = dataloader.dataset.samples[start_index_of_search_batch+n][0]

    # Sanity check
    for p_idx in range(n_prototypes):
        if proto_infos[p_idx]['min_dist'] > 1.0:
            print(proto_infos[p_idx])
    return proto_infos
