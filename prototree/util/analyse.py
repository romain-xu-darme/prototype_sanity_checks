...
def get_avg_path_length(tree: ProtoTree, info: dict, log: Log):
    ...
    if 'out_leaf_ix' not in info.keys():
        ...
        log.log_message("Tree with deterministic routing. Average path length is %s with std %s"
                        % (avg_depth, np.std(pred_depths)))
        log.log_message("Tree with deterministic routing. Longest path has length %s, shortest path has length %s"
...
def average_distance_nearest_image(project_info: dict, tree: ProtoTree, log: Log):
    ...
    if log:
        ...
        log.log_message("Average Euclidean distance and standard deviation from latent prototype "
                        "to nearest image patch: %s, %s" % (str(np.mean(distances)), str(np.std(distances))))
    return distances
...
def analyse_output_shape(tree: ProtoTree, trainloader: DataLoader, log: Log, device: str):
    tree.eval()
    with torch.no_grad():
    ...
    with torch.no_grad():
        if tree.depth <= 4:
            ...
            for leaf in tree._root.leaves:
                if leaf._log_probabilities:
                    log.log_message(str(leaf.index)+", " + str(leaf._dist_params)
        ...
        if len(leaf_labels.keys()) >= 2:
            ...
            log.log_message("Fraction changed pairs w.r.t previous epoch: %s"
                            % str(changed_prev/float(tree.num_leaves)))
            if len(leafs_higher_than) > 0:
                log.log_message("Fraction changed leafs with max > threshold w.r.t previous epoch: %s"
                                % str(changed_prev_higher/float(len(leafs_higher_than))))
    return leaf_labels


def analyse_ensemble(
        log, args, test_loader, device,
        trained_orig_trees,
    ...
    print("\nAnalysing and evaluating ensemble with %s trees of height %s..."
          % (len(trained_orig_trees), args.depth), flush=True)
    log.log_message("\n----------------------------------------------------------------"
                    "-------------------------------------------------")
    log.log_message("\nAnalysing and evaluating ensemble with %s trees of height %s..."
    ...
    log.log_message("Test accuracies of pruned and projected individual trees: %s"
    ...
    log.log_message("Mean and standard deviation of accuracies of pruned and projected individual trees:\n "
                    + "mean=" + str(np.mean(pruned_projected_test_accuracies))
                    + ", std=" + str(np.std(pruned_projected_test_accuracies)))
    ...
    log.log_message("Mean and standard deviation of fidelity of pruned and projected individual trees "
                    "with sample_max routing:\n "
    ...
    log.log_message("Mean and standard deviation of path length of pruned and projected individual trees "
                    "with sample_max routing:\n "
                    + "mean=" + str(np.mean(depths_sample_max)) + ", std=" + str(np.std(depths_sample_max)))
    log.log_message("Tree with sample_max deterministic routing. Longest path has length %s, "
                    "shortest path has length %s" % ((np.max(depths_sample_max)), np.min(depths_sample_max)))
    log.log_message("Mean and standard deviation of path length of pruned and projected individual trees "
                    "with greedy routing:\n "
                    + "mean=" + str(np.mean(depths_greedy)) + ", std=" + str(np.std(depths_greedy)))
    log.log_message("Tree with greedy deterministic routing. Longest path has length %s, "
                    "shortest path has length %s" % ((np.max(depths_greedy)), np.min(depths_greedy)))
